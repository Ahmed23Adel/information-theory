...
@checkProbs1
def huffman_alg(probs,original_length,codes=None):
    '''
    ...
    huffman_alg(trunc_lst,original_length = original_length, codes = codes) # I'm assuming that I work with list which is mutable
...
@checkSize
def calc_huffman_alg(probs):
    return huffman_alg(probs,len(probs))

    
def run_huffman(probs=None):
    run_compression_alg('Huffman', calc_huffman_alg)
